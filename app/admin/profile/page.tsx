...
export default function AdminProfilePage() {
  ...
  async function loadMe() {
    ...
    try {
      ...
      // 2) ถ้าไม่มี /api/me -> fallback user แรกจาก /api/admin/users
      if (!m) {
        const r2 = await fetch('/api/admin/users?q=', { cache: 'no-store' });
        const j2 = await safeJson(r2);
        if (!r2.ok || j2?.ok === false) throw new Error(j2?.message || 'load fallback failed');
        const list = j2?.data || [];
        if (!list.length) throw new Error('ไม่พบผู้ใช้ในระบบ');
        const u = list[0];
        m = {
          user_id: u.user_id,
          username: u.username ?? '',
  ...
  async function uploadAvatar(file: File) {
    ...
    fd.append('file', file);
    const res = await fetch(`/api/admin/users/${me.user_id}/avatar`, { method: 'POST', body: fd });
  ...
  async function saveProfile() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/users/${me.user_id}`, {
        method: 'PATCH',
  ...
  /* ===== Modal: Validate & Change Password ===== */
  ...
  async function submitChangePassword() {
    ...
    try {
      ...
      const json = await safeJson(res);
      if (!res.ok || json?.ok === false) throw new Error(json?.message || 'เปลี่ยนรหัสผ่านล้มเหลว');
      ...
      // แสดง error ใต้ช่องรหัสเดิม เพื่อความเข้าใจง่าย
  ...
  /* ===== UI ===== */
  return (
    <div className="min-h-[70vh] flex items-start justify-center py-8">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-4 flex items-center gap-2">
                ...
                <OutlineButton onClick={() => setPwOpen(true)}>เปลี่ยนรหัสผ่าน</OutlineButton>
              </div>
            </div>
          )}
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                ...
                <LabeledInput
                  ...
                  error={email && !looksLikeEmail(email) ? 'รูปแบบอีเมลไม่ถูกต้อง' : null}
                />
              </div>
